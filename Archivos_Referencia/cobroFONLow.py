...
#Mover la impresora aparte para que sea reutiizable
...
class FormularioOperacion:
    ...
    def agregarRegistroRFID(self):
        ...
        #IMPRESORA MOVER A UNA CLASE

        #aqui lo imprimimos
        p = Usb(0x04b8, 0x0202, 0)
        # p = Usb(0x04b8, 0x0e28, 0)#esta es la impresora con sus valores que se obtienen con lsusbÇ
    ...
    def consulta_por_folio(self):
        ...
        #Aqui hay un error con lo de dias, horas, minutos, segundos y total cobro
        ...
        if len(salida)>5:#None tiene 4 letras si es mayor a 5 es que tiene ya la fecha
            ...
            if ffeecha.days == 0 and horas_dentro == 0:
               ...
               self.entrypromo.focus()
                ...
                #if minutos <= 2:
                        #importe = 5
                 importe = ((ffeecha.days)*250 + (horas_dentro * 20)+(minutos)*5)
                 self.importe.set(importe)
...
###################### Fin de Pagina3 Inicio Pagina4 ###############################
    def Alta_Pensionado(self):
        ...
        self.boton1=tk.Button(self.labelframe1, text="Guardar", command=self.Guarda_Pensionado, width=12, height=1, anchor="center")#, background="red"
            ...
            #try:
                if len(numtarjeta) == 0 :
                    ...
                    if len(Existe) == 0 :
                        ...
                        self.PenApellMat.set("")
                        self.PenTelefonoObli.set("")
                        #self.PenTelefonoOpc.set("")   #REVISAR
                        self.PenCalle.set("")
                        self.PenColonia.set("")
                        self.PenCiudad.set("")
                        self.PenCP.set("")
    ...
    def Cobro_Pensionado(self):
            ...
            if len(numtarjeta) == 0 :
                ...
                #print("entro")
                tarjeta=int(numtarjeta)
                Existe=self.operacion1.ValidarID(tarjeta) #Existe = id_cliente de la Tabla de Pensionados
                if len(Existe) == 0 :
                    self.Monto.set("")
                    self.Vigencia.set("")
                    self.Estatus.set("")
